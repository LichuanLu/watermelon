...
function duster(data) {
  var configData = data;
  var input_path = path.normalize(cwd + configData.raw_dir);
  ...
  function compile_dust(filePath, curr, prev) {
    ...
    fs.readFile(filePath, function(err, data) {
      ...
      var templateId = path.basename(filePath, '.dust'),
        ...
        output_path = cwd + configData.pre_dir + filePath.substring(input_path.length);
...
fs.readFile(config, 'utf8', function(err, data) {
  ...
  var configDatas = JSON.parse(data);
  configDatas.forEach(function(configData) {
    duster(configData);
  });
});