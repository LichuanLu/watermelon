	...
	function(Lodash, CONSTANT, BaseController, View, ModalView, CommonView, ReqCmd, KefuEntity, CommonEntity) {
		...
		var KfController = BaseController.extend({
			initialize: function() {
				...
				//confirm audit
				ReqCmd.commands.setHandler("DoctorAuditListItemView:confirmHandler", Lodash.bind(function(doctorId) {

					if (doctorId) {
						var ModalModel = Backbone.Model.extend({});
						var modalModel = new ModalModel();
						modalModel.set("doctorId", doctorId);
				...
				//delete doctor audit
				ReqCmd.commands.setHandler("DoctorAuditListItemView:deleteHandler", Lodash.bind(function(doctorId) {
					if (doctorId) {
						var that = this;
						var ModalModel = Backbone.Model.extend({});
						var modalModel = new ModalModel();
				...
				ReqCmd.commands.setHandler("SharingListItemView:acceptLinkHandler", Lodash.bind(function(sharingId) {
					if (sharingId) {
						var that = this;
						var url = "/diagnosecomment/statuschang";
						var params = {
							id:sharingId,
							stauts:0
						}
